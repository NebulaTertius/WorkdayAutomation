...
INSERT INTO AI.LeaveBalanceQueueHistory
SELECT * FROM AI.LeaveBalanceQueue WHERE StatusCode IN ('New','Success')

DELETE FROM AI.LeaveBalanceQueue WHERE StatusCode IN ('New','Success')


INSERT INTO AI.LeaveTransactionQueueHistory
SELECT * FROM AI.LeaveTransactionQueue WHERE StatusCode IN ('Moved')
...
SELECT EmployeeCode,LeaveTypeCode,LeaveCode,SUM(TotalUnits) TotalUnits INTO ##LeaveTypesPerEmployee
...
GROUP BY EmployeeCode, LeaveTypeCode, LeaveCode
ORDER BY EmployeeCode, LeaveTypeCode, LeaveCode
...
WHERE e.EmployeeCode + lt.Code IN (SELECT DISTINCT EmployeeCode + LeaveTypeCode FROM ##LeaveTypesPerEmployee)
...
BEGIN	
	...
	PRINT CONVERT(varchar,@EmployeeCode) + ' - ' + CONVERT(varchar,@LeaveTypeCode)
	+ ' - ' + CONVERT(varchar,@TotalRunningUnits) + ' - ' + CONVERT(varchar,@CurrentUnitAdjust) + ' - ' + CONVERT(varchar,@CurrentSequence)	
...
PRINT CONVERT(varchar,@EmployeeCode) + ' - ' + CONVERT(varchar,@LeaveTypeCode)
	...
	SET @CurrentUnitAdjust = (SELECT TOP 1 CASE WHEN @TotalRunningUnits > BalanceIncludingPlanned THEN BalanceIncludingPlanned ELSE @TotalRunningUnits END [CurrentAdjust]
			...
			BEGIN
				INSERT INTO AI.LeaveBalanceQueue (EventCode,EventDescription,EventSequenceID,EmployeeCode,LeaveTypeCode,LeaveCode,UnitAdjustment,UnitOverride
				,StatusMessage,WarningMessage,ErrorMessage
				,DateCreated,StatusCode)
						SELECT 'New','New Leave Balance Adjustment',l.[Sequence],l.EmployeeCode,l.LeaveTypeCode,l.LeaveDefCode
